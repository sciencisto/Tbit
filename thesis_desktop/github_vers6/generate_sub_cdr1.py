...
def parse_commandline():  
    ...
    number = int(float(argv[7]))
    return path_dist, filename, path_binary, chunk_num, chunk_size, chunk_remain, number
...
def input_all_chunks(path, filename, binarypath, chunks, chunk_len, remain, number):
...
if __name__ == '__main__':
    ...
    path_dist, filename, path_binary, chunk_num, chunk_size, chunk_remain, number = parse_commandline()
    
    total = input_all_chunks(path_dist, filename, path_binary, chunk_num, chunk_size, chunk_remain, number)

        
    np.savetxt(number+"_CDR3_subsampled.csv", total, delimiter=',')
    print("--------------------------------")
