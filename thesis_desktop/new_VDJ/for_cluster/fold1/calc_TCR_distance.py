...
def dist_i_list_parallel(TCR_i, CDR3_list, V_list, dist_option_CDR3, dist_option_CDR1,  dist_option_CDR2, path):
    ...
    elif dist_option_CDR3 == "volume":
        dist_i = np.array([vol_distance(TCR_i[0], cdr3) for cdr3 in CDR3_list])
    ...
    elif dist_option_CDR1 == "volume":
        CDR1_volume = create_CDR12_dict(path+"Libraries/CDR1_volume.csv")
        dist_j = np.array([CDR1_volume[TCR_i[1], v] for v in V_list])
    ...
    elif dist_option_CDR2 == "volume":
        CDR2_volume = create_CDR12_dict(path+"Libraries/CDR2_volume.csv")
        dist_k = np.array([CDR2_volume[TCR_i[1], v] for v in V_list])
