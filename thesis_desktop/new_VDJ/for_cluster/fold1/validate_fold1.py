...
def dist_i_list_parallel(TCR_i, CDR3_list, V_list, path):
    ...
    dist_CDR1_RMSD = np.array([CDR1_RMSD[TCR_i[1], v] for v in V_list])*0.23
    
    CDR1_B62 = create_CDR12_dict(path+"Libraries/CDR1_blos62.csv")
    dist_CDR1_B62 = np.array([CDR1_B62[TCR_i[1], v] for v in V_list])*0.14
            
    CDR2_RMSD = create_CDR12_dict(path+"Libraries/CDR2_RMSD.csv")
    dist_CDR2_RMSD = np.array([CDR2_RMSD[TCR_i[1], v] for v in V_list])*0.12
    
    CDR2_B62 = create_CDR12_dict(path+"Libraries/CDR2_blos62.csv")
    dist_CDR2_B62 = np.array([CDR2_B62[TCR_i[1], v] for v in V_list])*0.18
    ...
    dist_CDR3_pam10 = np.array([dist_CDR3_alignment(TCR_i[0], cdr3, pam10, -30, -0.5, 
                        diagonal10, operator.ge)/1.5 for cdr3 in CDR3_list])*0.21
    
    dist_k3 = np.array([kmers_dist(TCR_i[0], cdr3, 3) for cdr3 in CDR3_list])*0.12

    distance = (dist_CDR1_RMSD + dist_CDR1_B62 + dist_CDR2_RMSD +
                dist_CDR2_B62 + dist_CDR3_pam10 + dist_k3)
