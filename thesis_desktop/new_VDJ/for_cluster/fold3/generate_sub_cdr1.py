...
def parse_commandline():  
    ...
    python3 generate_sub_cdr1.py '/work1/s111518/new_VDJ' 'Currated_VDJdb.tsv'  'work1/s111518/new_VDJ/Binary_group' 87 50 46
	"""
    path_dist = argv[1]
    filename = argv[2]
    path_binary = argv[3]
...
def getting_one_line_from_Each(filepath, filename, line, binarypath):
    empty = []
    CDR1_method_list = ["TM", "RMSD", "Blosum62", "hydrophobicity"]
    for j in range(len(CDR1_method_list)):
        y = import_lines("%s/CDR1dist_%s%s" % (filepath, CDR1_method_list[j], filename), line)
        empty.append(y)
    print("reading lines...")
    ...
    df = df.transpose()
    df.columns = CDR1_method_list + ["Bin"]
    return df
...
#    log_data = log_Regress(total)    
    ...
    np.savetxt("CDR1_subsampled.csv", total, delimiter=',')
    print("--------------------------------")
