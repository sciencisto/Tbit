...
def dist_i_list_parallel(TCR_i, CDR3_list, V_list, path):
    ...
    dist_CDR1_TM = np.array([CDR1_TM[TCR_i[1], v] for v in V_list])*0.14
    
    CDR1_B62 = create_CDR12_dict(path+"Libraries/CDR1_blos62.csv")
    dist_CDR1_B62 = np.array([CDR1_B62[TCR_i[1], v] for v in V_list])*0.12
            
    CDR2_RMSD = create_CDR12_dict(path+"Libraries/CDR2_RMSD.csv")
    dist_CDR2_RMSD = np.array([CDR2_RMSD[TCR_i[1], v] for v in V_list])*0.14
    
    CDR2_TM = create_CDR12_dict(path+"Libraries/dist_CDR2_TM_1.csv")
    dist_CDR2_TM = np.array([CDR2_TM[TCR_i[1], v] for v in V_list])*0.13
    ...
    dist_k3 = np.array([kmers_dist(TCR_i[0], cdr3, 3) for cdr3 in CDR3_list])*0.18
    
    distance = (dist_CDR1_TM + dist_CDR1_B62 + dist_CDR2_RMSD +
                dist_CDR2_TM + dist_CDR3_pam10 + dist_k3)
