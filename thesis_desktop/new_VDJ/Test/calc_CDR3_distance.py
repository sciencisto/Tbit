...
import math
from calc_substitution_matrices import Blosum62, pam10, Blosum45, Blosum50_
##############################################################################
...
def CDRdist(TCR1_CDR3, TCR2_CDR3, metric, gappenalty, gap_extend):
        
    s1 = Protein(TCR1_CDR3)
    s2 = Protein(TCR2_CDR3)
        
    sij = local_pairwise_align_protein(s1, s2, gappenalty, gap_extend, substitution_matrix=metric)
    ij = sij[1]
    sii = local_pairwise_align_protein(s1, s1, gappenalty, gap_extend, substitution_matrix=metric)
    ii = sii[1]
    sjj = local_pairwise_align_protein(s2, s2, gappenalty, gap_extend, substitution_matrix=metric)
    jj = sjj[1]
    
    distance = 1 - math.sqrt(ij**2/(ii*jj))
