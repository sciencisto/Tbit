...
def input_all_chunks(path, filename, binarypath, chunks, chunk_len, remain, CDR):
    ...
    print("Each chunk number is send to me read in linewise")
    for i in range(chunks):
        if i in range(chunks-1):
            new = over_all_lines(path,"/%s_"%(i)+file_n+"_dmat.tsv", binarypath, chunk_len, CDR)
            df_new = pd.concat([df, new], ignore_index = True)
        else:
            new1 = over_all_lines(path,"/%s_"%(i)+file_n+"_dmat.tsv", binarypath, remain, CDR)
